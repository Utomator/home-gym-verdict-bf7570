# /content-guide — author a `guide` page (affiliate)

Author ONE on-recipe `guide` page for this affiliate site, following the scaffold below exactly. Use the canonical `category_slug` from the platform claim step verbatim. Read DESIGN.md first; match the site's design language.

> Selection recipe: this archetype plans ~3 `guide` page(s).
